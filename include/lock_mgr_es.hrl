...
-define(LOCK_PART_TO_DC_OFFSET, 0).               % non_neg_integer()
                                                  % Defines with how many lock parts in addition to those of "LOCK_PART_TO_DC_FACTOR" a new es_lock is initiated with
...
%-define(WAIT_FOR_SHARED_LOCKS1,1).               %                    1 - When acquiring an exclusive lock, the transaction will wait for data updates of all shared and exclusive lock commits
                                                  %                        For shared locks it will also wait for all commits using these as exclusive locks, and commits using these as shared locks if committed on the same DC
-define(WAIT_FOR_SHARED_LOCKS2, 1).               %                    2 - For every exclusive lock requested, the transaction will wait for data updates of all shared and exclusive lock commits
                                                  %                        For every shared lock requested, the transaction will only wait for data updates of commits using that lock as an exclusive one
...
-define(GET_LOCKS_FINAL_TRY_OPTION_ES, true).     % Decides if the clocksi_interactive_coord waits for GET_LOCKS_FINAL_TRY_WAIT after a lock could not be acquired.
                                                  % This is useful if GET_LOCKS_INTERVAL_ES is set very low in comparison to the time needed by DCs to communicate.
-define(GET_LOCKS_FINAL_TRY_WAIT_ES, 800).        % Defines how long it waits until it does the last attempt to get the locks.
