...
-define(LOCK_REQUEST_TIMEOUT, 250000).     % Locks requested by other DCs ( in microseconds -- 1 000 000 equals 1 second)
...
-define(LOCK_REQUIRED_TIMEOUT,300000).     % Locks requested by transactions of this DC (in microseconds -- 1 000 000 equals 1 second)
...
-define(LOCK_TRANSFER_FREQUENCY,200).       % Sets how long lock_mgr waits between each time it sends requested locks to other DCs (in milliseconds -- 1 000 equals 1second)
...
-define(GET_LOCKS_FINAL_TRY_WAIT,400).       % Defines how long it waits untill it does the last attempt to get the locks.