...
-export([confirm/0,
     ...
     spawn_read/4,
	 ec_test_prepare/1,
     ec_tx_noclock_test/1,
     ec_single_key_update_read_test/1,
     ec_multiple_key_update_read_test/1,
     ec_test_certification_check/1,
     ec_multiple_test_certification_check/1,
     ec_multiple_read_update_test/1,
     ec_concurrency_test/1,
	 spawn_com/2]).
...
ec_test1(Nodes) ->
    ...
    Result0=rpc:call(FirstNode, ec_antidote, ec_execute_tx,
                    [[]]),
    ?assertMatch({ok, _}, Result0),
    Result1=rpc:call(FirstNode, ec_antidote, ec_execute_tx,
                    [[]]),
    ?assertMatch({ok, _}, Result1),

    % A simple read returns empty
    Result11=rpc:call(FirstNode, ec_antidote, ec_execute_tx,
                    [
                     [{read, {Key1, Type}}]]),
    ...
    %% Read what you wrote
    Result2=rpc:call(FirstNode, ec_antidote, ec_execute_tx,
                    [
                      [{read, {Key1, Type}},
    ...
    Result3=rpc:call(FirstNode, ec_antidote, ec_execute_tx,
                    [
                     [{read, {Key1, Type}},
                      {read, {Key2, Type}}]]),
    ...
    Result5=rpc:call(FirstNode, ec_antidote, ec_execute_tx,
                    [
                     [{update, {Key1, Type, {increment, a}}},
                      {update, {Key1, Type, {increment, a}}}]]),
    ?assertMatch({ok,_}, Result5),

    Result6=rpc:call(FirstNode, ec_antidote, ec_execute_tx,
                    [
                     [{read, {Key1, Type}}]]),
...
ec_test2(Nodes) ->
    ...
    Key2=ec_test2_key2,
    Key3=ec_test2_key3,
    {ok,TxId}=rpc:call(FirstNode, ec_antidote, ec_istart_tx, []),
    ReadResult0=rpc:call(FirstNode, ec_antidote, ec_iread,
                         [TxId, Key1, riak_dt_pncounter]),
    ?assertEqual({ok, 0}, ReadResult0),
    WriteResult=rpc:call(FirstNode, ec_antidote, ec_iupdate,
                         [TxId, Key1, Type, {increment, 4}]),
    ?assertEqual(ok, WriteResult),
    ReadResult=rpc:call(FirstNode, ec_antidote, ec_iread,
                        [TxId, Key1, riak_dt_pncounter]),
    ?assertEqual({ok, 1}, ReadResult),
    WriteResult1=rpc:call(FirstNode, ec_antidote, ec_iupdate,
                          [TxId, Key2, Type, {increment, 4}]),
    ?assertEqual(ok, WriteResult1),
    ReadResult1=rpc:call(FirstNode, ec_antidote, ec_iread,
                         [TxId, Key2, riak_dt_pncounter]),
    ?assertEqual({ok, 1}, ReadResult1),
    WriteResult2=rpc:call(FirstNode, ec_antidote, ec_iupdate,
                          [TxId, Key3, Type, {increment, 4}]),
    ?assertEqual(ok, WriteResult2),
    ReadResult2=rpc:call(FirstNode, ec_antidote, ec_iread,
                         [TxId, Key3, riak_dt_pncounter]),
    ?assertEqual({ok, 1}, ReadResult2),
    CommitTime=rpc:call(FirstNode, ec_antidote, ec_iprepare, [TxId]),
    ?assertMatch({ok, _}, CommitTime),
    End=rpc:call(FirstNode, ec_antidote, ec_icommit, [TxId]),
    ?assertMatch({ok, {_Txid, _CausalSnapshot}}, End),
    {ok,{_Txid, CausalSnapshot}} = End,
    ReadResult3 = rpc:call(FirstNode, ec_antidote, ec_read,
...
ec_test3(Nodes) ->
    ...
    ReadResult2=rpc:call(FirstNode, ec_antidote, ec_iread,
                         [TxId, Key3, riak_dt_pncounter]),
    ?assertEqual({ok, 1}, ReadResult2),
    End=rpc:call(FirstNode, ec_antidote, ec_full_icommit, [TxId]),
    ?assertMatch({ok, {_Txid, _CausalSnapshot}}, End),
    {ok,{_Txid, CausalSnapshot}} = End,
    ReadResult3 = rpc:call(FirstNode, ec_antidote, ec_read,
                           [CausalSnapshot, Key1, Type]),
...
ec_test_prepare(Nodes) ->
    FirstNode = hd(Nodes),
    lager:info("Test prepare started"),
    Type = riak_dt_pncounter,

    Key1=ec_test_prepare_key1,
    ...
    {ok,TxIdRead}=rpc:call(FirstNode, ec_antidote, ec_istart_tx, []),    

    timer:sleep(3000),

    {ok,TxId1}=rpc:call(FirstNode, ec_antidote, ec_istart_tx, []),
    WriteResult1=rpc:call(FirstNode, ec_antidote, ec_iupdate,
    ...
    ?assertEqual({ok, 1}, ReadResult1),
    CommitTime1=rpc:call(FirstNode, ec_antidote, ec_iprepare, [TxId1]),
    ?assertMatch({ok, _}, CommitTime1),
       
    spawn(?MODULE, spawn_com, [FirstNode, TxId]),

    ReadResultR=rpc:call(FirstNode, ec_antidote, ec_iread,
			 [TxIdRead, Key1, riak_dt_pncounter]),
    ?assertEqual({ok, 1}, ReadResultR),

    End1=rpc:call(FirstNode, ec_antidote, ec_icommit, [TxId1]),
...
spawn_com(FirstNode, TxId) ->
    timer:sleep(3000),
    End1 = rpc:call(FirstNode, ec_antidote, ec_icommit, [TxId]),
    ?assertMatch({ok, {_Txid, _CausalSnapshot}}, End1).
...
ec_tx_noclock_test(Nodes) ->
    FirstNode = hd(Nodes),
    Key = ec_tx_noclock_test_key1,
    Type = riak_dt_pncounter,
    {ok,TxId}=rpc:call(FirstNode, ec_antidote, ec_istart_tx, []),
    ReadResult0=rpc:call(FirstNode, ec_antidote, ec_iread,
                         [TxId, Key, riak_dt_pncounter]),
    ?assertEqual({ok, 0}, ReadResult0),
    WriteResult0=rpc:call(FirstNode, ec_antidote, ec_iupdate,
    ...
    ReadResult1 = rpc:call(FirstNode, ec_antidote, ec_read,
                           [Key, riak_dt_pncounter]),
    {ok, {_, ReadSet1, _}}= ReadResult1,
    ?assertMatch([1], ReadSet1),

    FirstNode = hd(Nodes),
    WriteResult1 = rpc:call(FirstNode, ec_antidote, ec_bulk_update,
                            [[{update, {Key, Type, {increment, a}}}]]),
    ?assertMatch({ok, _}, WriteResult1),
    ReadResult2= rpc:call(FirstNode, ec_antidote, ec_read,
                          [Key, riak_dt_pncounter]),
...
ec_single_key_update_read_test(Nodes) ->
    ...
    FirstNode = hd(Nodes),
    Key = ec_single_key_update_read_test_key1,
    Type = riak_dt_pncounter,
    Result= rpc:call(FirstNode, ec_antidote, ec_bulk_update,
                     [
                      [{update, {Key, Type, {increment, a}}},
                       {update, {Key, Type, {increment, b}}}]]),
    ?assertMatch({ok, _}, Result),
    {ok,{_,_,CommitTime}} = Result,
    Result2= rpc:call(FirstNode, ec_antidote, ec_read,
...
ec_multiple_key_update_read_test(Nodes) ->
    Firstnode = hd(Nodes),
    Type = riak_dt_pncounter,
    Key1 = ec_multiple_key_update_read_test_key1,
    Key2 = ec_multiple_key_update_read_test_key2,
    Key3 = ec_multiple_key_update_read_test_key3,
    Ops = [{update, {Key1, Type, {increment,a}}},
           {update, {Key2, Type, {{increment,10},a}}},
           {update,{Key3, Type, {increment,a}}}],
    Writeresult = rpc:call(Firstnode, ec_antidote, ec_bulk_update,
                           [Ops]),
    ?assertMatch({ok,{_Txid, _Readset, _Committime}}, Writeresult),
    {ok,{_Txid, _Readset, Committime}} = Writeresult,
    {ok,{_,[ReadResult1],_}} = rpc:call(Firstnode, ec_antidote, ec_read,
                                        [Committime, Key1, riak_dt_pncounter]),
    {ok,{_,[ReadResult2],_}} = rpc:call(Firstnode, ec_antidote, ec_read,
                                        [Committime, Key2, riak_dt_pncounter]),
    {ok,{_,[ReadResult3],_}} = rpc:call(Firstnode, ec_antidote, ec_read,
...
ec_test_read_time(Nodes) ->
    ...
    {ok,TxId1}=rpc:call(LastNode, ec_antidote, ec_istart_tx, []),
    ...
    ReadResult1=rpc:call(LastNode, ec_antidote, ec_iread,
                         [TxId1, Key1, riak_dt_pncounter]),
    lager:info("Tx2 Reading..."),
    ...
    CommitTime1=rpc:call(LastNode, ec_antidote, ec_iprepare, [TxId1]),
    ?assertMatch({ok, _}, CommitTime1),
    lager:info("Tx2 sent prepare, got commitTime=..., id : ~p", [CommitTime1]),
    End1=rpc:call(LastNode, ec_antidote, ec_icommit, [TxId1]),
    ?assertMatch({ok, _}, End1),
...
ec_test_read_wait(Nodes) ->
    ...
    lager:info("Tx1 Writing..."),
    ?assertEqual(ok, WriteResult),
    {ok, CommitTime}=rpc:call(FirstNode, ec_antidote, ec_iprepare, [TxId]),
    lager:info("Tx1 sent prepare, got commitTime=..., id : ~p", [CommitTime]),
...
spawn_read(LastNode, TxId, Return, Key) ->
    ReadResult=rpc:call(LastNode, ec_antidote, ec_iread,
                        [TxId, Key, riak_dt_pncounter]),
...
ec_test_certification_check(Nodes) ->
    ...
    {ok,TxId1}=rpc:call(LastNode, ec_antidote, ec_istart_tx, []),
    lager:info("Tx2 Started, id : ~p", [TxId1]),
    WriteResult1=rpc:call(LastNode, ec_antidote, ec_iupdate,
...
ec_multiple_test_certification_check(Nodes) ->
    ...
    Key1 = ec_multiple_test_certification_check_key1,
    Key2 = ec_multiple_test_certification_check_key2,
    Key3 = ec_multiple_test_certification_check_key3,
    LastNode= lists:last(Nodes),
    lager:info("Node1: ~p", [FirstNode]),
    lager:info("LastNode: ~p", [LastNode]),
    Type = riak_dt_pncounter,
    ...
    ?assertEqual(ok, WriteResult),
    WriteResultb=rpc:call(FirstNode, ec_antidote, ec_iupdate,
                          [TxId, Key2, Type, {increment, 1}]),
    lager:info("Tx1 Writing 2..."),
    ?assertEqual(ok, WriteResultb),
    WriteResultc=rpc:call(FirstNode, ec_antidote, ec_iupdate,
...
ec_multiple_read_update_test(Nodes) ->
    ...
    Key = get_random_key(),
    NTimes = 100,
    {ok,Result1} = rpc:call(Node, ec_antidote, read,
                       [Key, riak_dt_pncounter]),
    lists:foreach(fun(_)->
                          read_update_test(Node, Key) end,
                  lists:seq(1,NTimes)),
    {ok,Result2} = rpc:call(Node, ec_antidote, read,
                       [Key, riak_dt_pncounter]),
...
read_update_test(Node, Key) ->
    ...
    {ok,Result1} = rpc:call(Node, ec_antidote, read,
                       [Key, Type]),
    {ok,_} = rpc:call(Node, ec_antidote, ec_bulk_update,
                      [[{update, {Key, Type, {increment,a}}}]]),
    {ok,Result2} = rpc:call(Node, ec_antidote, read,
                       [Key, Type]),
...
ec_concurrency_test(Nodes) ->
    ...
    Key = conc,
    {ok, TxId1} = rpc:call(Node, ec_antidote, ec_istart_tx, []),
    rpc:call(Node, ec_antidote, ec_iupdate,
             [TxId1, Key, riak_dt_gcounter, {increment, ucl}]),
    rpc:call(Node, ec_antidote, ec_iprepare, [TxId1]),
    {ok, TxId2} = rpc:call(Node, ec_antidote, ec_istart_tx, []),
    Pid = self(),
    spawn( fun() ->
                   rpc:call(Node, ec_antidote, ec_iupdate,
                            [TxId2, Key, riak_dt_gcounter, {increment, ucl1}]),
                   rpc:call(Node, ec_antidote, ec_iprepare, [TxId2]),
                   {ok,_}= rpc:call(Node, ec_antidote, ec_icommit, [TxId2]),
                   Pid ! ok
           end),

    {ok,_}= rpc:call(Node, ec_antidote, ec_icommit, [TxId1]),
     receive
         ok ->
             Result= rpc:call(Node,
                              ec_antidote, read, [Key, riak_dt_gcounter]),
