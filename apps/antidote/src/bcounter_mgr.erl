...
handle_cast({transfer, {Key, Amount, Requester}}, #state{last_transfers = LT} = State) ->
    ...
    case can_process(Key, Requester, NewLT) of
        true ->
            ...
            {noreply, State#state{
                last_transfers = orddict:store({Key, Requester}, erlang:timestamp(), NewLT)
            }};
        _ ->
...
queue_request(_Key, 0, RequestsQueue) ->
    RequestsQueue;
queue_request(Key, Amount, RequestsQueue) ->
    QueueForKey =
        case orddict:find(Key, RequestsQueue) of
            {ok, Value} -> Value;
            error -> orddict:new()
        end,
        ...
        fun({RemoteId, AvailableRemotely}, Remaining0) ->
            case Remaining0 > 0 of
                true when AvailableRemotely > 0 ->
                    ToRequest =
                        case AvailableRemotely - Remaining0 >= 0 of
                            true -> Remaining0;
...
do_request(MyDCId, RemoteId, Key, Amount) ->
    ...
    BinaryMsg = term_to_binary(
        {request_permissions, {transfer, {Key, Amount, MyDCId}}, LocalPartition, MyDCId, RemoteId}
    ),
    inter_dc_query_dealer:perform_request(
        ?BCOUNTER_REQUEST,
        {RemoteId, LocalPartition},
        BinaryMsg,
        fun bcounter_mgr:request_response/1
    ).
...
cancel_consecutive_req(LastTransfers, Period) ->
    ...
    orddict:filter(
        fun(_, Timeout) ->
            timer:now_diff(Timeout, CurrTime) < Period
        end,
        LastTransfers
    ).
