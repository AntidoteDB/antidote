...
start_timer(Partition) ->
    dc_utilities:call_vnode_sync(Partition, inter_dc_log_sender_vnode_master, {start_timer}).
...
update_last_log_id(Partition, OpId) ->
    dc_utilities:call_vnode_sync(
        Partition, inter_dc_log_sender_vnode_master, {update_last_log_id, OpId}
    ).
...
del_timer(State = #state{timer = none}) ->
    State;
        ...
        true ->
            ...
            case Node of
                MyNode ->
                    ...
                    State1#state{
                        timer = riak_core_vnode:send_command_after(?HEARTBEAT_PERIOD, ping)
                    };
                _Other ->
                    State
            end;
