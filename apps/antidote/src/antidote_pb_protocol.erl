...
handle(Socket, Transport, Msg) ->
    ...
    try
        ...
        ExceptionType:Error:StackTrace ->
            ...
            ?LOG_ERROR("Error ~p: ~p~n~p~nWhen handling request ~p~n", [
                ExceptionType, Error, StackTrace, DecodedMessage
            ]),
            ...
            MessageStr = erlang:iolist_to_binary(
                io_lib:format("~P: ~P~n~P~n", [ExceptionType, 9001, Error, 9001, StackTrace, 9001])
            ),
