...
handle_info(
    periodic_expensive_update,
    State = #state{timer_expensive = ExpensiveTimer, monitored_processes = Monitored}
) ->
    %% ?
    ...
    case application:get_env(antidote, extended_stats) of
        {ok, true} ->
            ...
            case Continue of
                true ->
                    Timer = erlang:send_after(?INTERVAL_LONG, self(), periodic_expensive_update);
                _ ->
                    Timer = undefined
            end,
...
update_processes_info(Monitored) ->
    ...
    NewMonitored = maps:fold(
        fun(Name, Messages, MonitorAcc) ->
            case lists:any(fun(E) -> E == Name end, MonitorAcc) of
                ...
                _ ->
                    ...
                    case Messages > ?QUEUE_LENGTH_THRESHOLD of
                        true ->
                            ?LOG_WARNING(
                                "New process has a message queue and is now being monitored ~p: ~p",
                                [Name, Messages]
                            ),
                            ?STATS({process_message_queue_length, Name, Messages}),
                            ?STATS({process_reductions, Name, maps:get(Name, ReductionsMap)}),
    ...
    case TimeMs > ?TIME_METRIC_COLLECTION_THRESHOLD_MS of
        true ->
            ?LOG_ALERT(
                "System metric process collection took too long (~p ms over ~p ms threshold), turning process info collection off",
                [TimeMs, ?TIME_METRIC_COLLECTION_THRESHOLD_MS]
            ),
            {false, NewMonitored};
        _ ->
