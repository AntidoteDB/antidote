...
object_log_state_test(Config) ->    
    ...
    CommitTime = add_set(FirstNode, {Key,Bucket}, lists:seq(1,15), vectorclock:new()),
    ...
    CommitTime2 = add_set(FirstNode, {Key,Bucket}, lists:seq(16,30), CommitTime),
...
check_orset_ops([Val|Rest1],
		[{_Id,#clocksi_payload{
			 key = KeyBucket,
			 type = antidote_crdt_orset,
			 op_param = {add, {Val, [Binary]}}}}
		 | Rest2],
		KeyBucket) when is_binary(Binary) ->
...
add_set(_FirstNode, _Key, [], Commit) ->
    Commit;
add_set(FirstNode, Key, [First|Rest], PrevCommit) ->
    {WriteResult, {_Txid, _Val, Commit}} = rpc:call(FirstNode, antidote, clocksi_execute_tx,
						   [[{read, {Key, antidote_crdt_orset}},
						     {update, {Key, antidote_crdt_orset, {add, First}}}]]),
    ?assertMatch(ok, WriteResult),
    add_set(FirstNode, Key, Rest, vectorclock:max([PrevCommit,Commit])).
