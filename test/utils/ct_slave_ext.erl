...
%%----------------------------------------------------------------------
%% This is an adaptation of ct_slave from the Erlang standard library.
%% It adds the following features:
%%  - Configuration to set the working directory of a slave
%%----------------------------------------------------------------------
-module(ct_slave_ext).
...
fetch_options(Options) ->
    ...
    WorkingDir = get_option_value(cd, Options ,false),
    #options{username=UserName, password=Password,
        ...
        stop_timeout=StopTimeout, working_dir=WorkingDir}.
...
get_cmd(Node, Flags) ->
    ...
    "erl -setcookie "++ atom_to_list(Cookie) ++
...
spawn_local_node(Node, Options) ->
    ...
    spawn(fun() ->
        Port = open_port({spawn, Cmd}, PortOpds),
        Cwd = case Options#options.working_dir of
            false ->
                {ok, D} = file:get_cwd(),
                D;
            D -> D
        end,
        log_output(Node, Port, Cwd ++ "/stdio.log")
    end).
