...
-module(eiger_inter_dc_repl_vnode).
...
init([Partition]) ->
    ...
    {ok, Reader} = eiger_transaction_reader:init(Partition, DcId),
    {ok, #state{partition=Partition,
                dcid=DcId,
                reader = Reader}}.

handle_command(trigger, _Sender, State=#state{partition=_Partition,
                                              reader=Reader}) ->
    ...
    NewState = case DCs of
        ...
        DCs ->
            {NewReaderState, Transactions} =
                eiger_transaction_reader:get_next_transactions(Reader),
