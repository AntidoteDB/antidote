...
%% --------------------------------------------------------------------------
%%
%% counter_b: A convergent, replicated, operation-based bounded counter.
%%
%% --------------------------------------------------------------------------
...
-export_type([counter_b/0, binary_counter_b/0, counter_b_op/0, id/0]).

-opaque counter_b() :: {orddict:orddict(), orddict:orddict()}.
-type binary_counter_b() :: binary().
-type counter_b_op() :: counter_b_anon_op() | counter_b_src_op().
-type counter_b_anon_op() :: {transfer, {pos_integer(), id(), id()}} |
    {increment, {pos_integer(), id()}} | {decrement, {pos_integer(), id()}}.
-type counter_b_src_op() :: {counter_b_anon_op(), id()}.
-opaque id() :: term. %% A replica's identifier.

%% @doc Return a new, empty `counter_b()'.
-spec new() -> counter_b().
new() ->
    {orddict:new(), orddict:new()}.

%% @doc Return the available permissions of replica `Id' in a `counter_b()'.
-spec localPermissions(id(), counter_b()) -> non_neg_integer().
...
%% @doc Return the total available permissions in a `counter_b()'.
-spec permissions(counter_b()) -> non_neg_integer().
permissions({P, D}) ->
...
%% @doc Return the read value of a given `counter_b()', itself.
-spec value(counter_b()) -> counter_b().
...
%% and the third parameter is a `counter_b()' which holds the current snapshot.
%%
...
%%
%% Return the resulting `counter_b()' after applying the operation.
-spec update(term(), counter_b()) -> {ok, counter_b()}.
...
%% doc Return the binary representation of a `counter_b()'.
-spec to_binary(counter_b()) -> binary().
to_binary(C) -> term_to_binary(C).

%% doc Return a `counter_b()' from its binary representation.
-spec from_binary(binary()) -> {ok, counter_b()}.
...
%% Tests creating a new `counter_b()'.
new_test() ->
    ?assertEqual({[], []}, new()).
...
value_test() ->
    %% Test on `counter_b()' resulting from applying all kinds of operation.
    Counter0 = new(),
    Counter1 = apply_op({increment, {10, r1}}, Counter0),
    Counter2 = apply_op({decrement, {6, r1}}, Counter1),
    Counter3 = apply_op({transfer, {2, r2, r1}}, Counter2),
    %% Assert `value()' returns `counter_b()' itself.
...
binary_test() ->
    ...
    Counter2 = apply_op({decrement, {6, r1}}, Counter1),
    Counter3 = apply_op({transfer, {2, r2, r1}}, Counter2),
    %% Assert marshaling and unmarshaling holds the same `counter_b()'.
