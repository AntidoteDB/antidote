...
init([Partition]) ->
    ...
    {ok, #state{partition=Partition, %active_tx=ActiveTx,
...
handle_command({get_active_txns},_Sender, #state{prepared_tx=Prepared, partition = Partition}=State) ->
    %%ActiveTxs :: [{active,{TxId, Preparetime}}]
