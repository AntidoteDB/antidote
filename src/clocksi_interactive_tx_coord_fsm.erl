...
prepare(timeout, SD0=#state{
    ...
    case length(UpdatedPartitions) of
        0->
            ...
            SD0#state{state=committing, commit_time=SnapshotTime}, 0};
        1-> 
            clocksi_vnode:single_commit(UpdatedPartitions, Transaction),
...
-spec get_snapshot_time(dcid()) -> {ok, snapshot_time()} | {error, reason()}.
get_snapshot_time(DcId) ->
