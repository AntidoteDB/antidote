...
handle_command({log_event, Operation}, _Sender, State) ->
  ...
  case Result of
    {ok, Txn} -> handle_transaction(State#state.partition, Txn);
    none -> none
  end,
...
handle_transaction(Partition, Txn) ->
  new_inter_dc_pub:broadcast_transaction(Partition, Txn).
  %%CommitOperation = lists:last(Txn),
  %%{{DcId, _TxCommitTime}, _Snapshot} = CommitOperation#log_record.op_payload,

  %% If the transaction originated from this DC, it should be sent to other DCs.
  %%case DcId =:= dc_utilities:get_my_dc_id() of
  %%  true -> new_inter_dc_pub:broadcast(Txn);
  %%  false -> ok
  %%end.
