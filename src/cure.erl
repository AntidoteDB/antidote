...
update_objects(ClientCausalVC, _Properties, Updates, StayAlive) ->
    FormattedUpdates = format_update_params(Updates),
    start_static_transaction(update_objects, FormattedUpdates, StayAlive, ClientCausalVC).

%% @doc The following function is called by the static versions of read and update_objects
%%      to execute a static transaction.
-spec start_static_transaction(update_objects | read_objects, list(), boolean(), vectorclock(), atom())
                              -> {ok, snapshot_time()} | {error, reason()}.
start_static_transaction(TransactionKind, ListOfOperations, StayAlive, ClientCausalVC, UpdateClock) ->
    ...
    case TxPid of
        undefined ->
            {ok, _CoordFSM} = clocksi_interactive_tx_coord_sup:start_fsm([self(),
                                                                          ClientCausalVC,
                                                                          UpdateClock,
                                                                          StayAlive,
                                                                          {TransactionKind, ListOfOperations}]);
        TxPid ->
            ok = gen_fsm:send_event(TxPid, {start_tx, self(), ClientCausalVC,
                                            UpdateClock, {TransactionKind, ListOfOperations}})
    end,
    receive
        Reply ->
            Reply
...
start_static_transaction(TransactionKind, ListOfOperations, StayAlive, ClientCausalVC) ->
    start_static_transaction(TransactionKind, ListOfOperations, StayAlive, ClientCausalVC, update_clock).
                ...
                {ok, gr} ->
                    case FormattedObjects of
                        ...
                        [_|_] -> %% Read Multiple objects  = read from a snapshot
                            ...
                            case gr_snapshot_read(Clock, FormattedObjects) of
...
gr_snapshot_read(ClientClock, Args) ->
    ...
    case Dt =< GST of
        true ->
            ...
            start_static_transaction(read_objects, Args, false, SnapshotTime, no_update_clock);
        false ->
            timer:sleep(10),
            gr_snapshot_read(ClientClock, Args)
    end.
