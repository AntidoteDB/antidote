...
%%
%% -------------------------------------------------------------------
-module(ec_ec_materializer_vnode).
      ...
      case (LatestCommitTime = ignore or vectorclock:is_greater_than(Op1CommitTime, LatestCommitTime)) of
        true ->
          case ec_materializer:materialize_eager(Type, LatestSnapshot, Op1) of
            UpdatedSnapshot ->
              internal_store_ss(Key, UpdatedSnapshot, Op1CommitTime, OpsCache, SnapshotCache);
            {error, Reason} ->
              {error, Reason}
          end
      end
...
-spec snapshot_insert(key(), snapshot(), snapshot_time(), cache_id()) -> true.
...
-spec op_insert(key(), clocksi_payload(), cache_id()) -> true.
op_insert(Key, DownstreamOp, OpsCache) ->
  case ets:lookup(OpsCache, Key) of
    [] ->
      {_, CommitTime} = DownstreamOp#clocksi_payload.commit_time,
      ets:insert(OpsCache, {Key, {CommitTime, DownstreamOp}});
    [{_, {StoredCommitTime, _}}] ->
      {_, OpCommitTime} = DownstreamOp#clocksi_payload.commit_time,
      case OpCommitTime > StoredCommitTime of
        true ->
          ets:insert(OpsCache, {Key, {DownstreamOp#clocksi_payload.commit_time, DownstreamOp}});
        _ ->
          true
      end
  end.
