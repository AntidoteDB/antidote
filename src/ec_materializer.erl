...
-spec materialize(type(), snapshot(),
		  ...
		  snapshot_time(),
		  [ec_payload()], txid() | ignore) -> {ok, snapshot(), 
...
%%      it, or an error.
-spec apply_operations(type(), snapshot(), [ec_payload()]) -> {ok, snapshot()} | {error, reason()}.
apply_operations(_Type,Snapshot,[]) ->
    {ok, Snapshot};
apply_operations(Type,Snapshot,[Op | Rest]) ->
    case materializer:update_snapshot(Type, Snapshot, Op#ec_payload.op_param) of
	{ok, NewSnapshot} -> 
...
-spec materialize_intern(type(), 
		  ...
		  snapshot_time() | ignore,
		  boolean()) ->
			 {ok,[ec_payload()],snapshot_time()|ignore,boolean()}.
...
materialize_intern(Type, OpList, SnapshotCommitTime, MinSnapshotTime, [Op|Rest], TxId, LastOpCt, NewSS) ->
    Result = case Type == Op#ec_payload.type of
		 true ->
		     OpCom=Op#ec_payload.commit_time,
		     OpSS=Op#ec_payload.snapshot_time,
...
-spec is_op_in_snapshot(txid(), ec_payload(), commit_time(), snapshot_time(), snapshot_time(),
...
is_op_in_snapshot(TxId, Op, {OpDc, OpCommitTime}, OperationSnapshotTime, SnapshotTime, LastSnapshot, PrevTime) ->
    ...
    case materializer_vnode:belongs_to_snapshot_op(
	   LastSnapshot,{OpDc,OpCommitTime},OperationSnapshotTime) or (TxId == Op#ec_payload.txid) of
	true ->
