...
-spec downstream(mvreg_op(), mvreg()) -> {ok, term()}.
downstream({assign, {Value, Actor}}, MVReg) ->
...
downstream({assign, {Value, TS, Actor}}, MVReg) ->
    case larger_than(TS, Actor, MVReg) of
        true ->
            VV = inc_vv(MVReg, Actor),
            NewMVReg = init(Value, VV),
            downstream({assign, {Value, TS, Actor}}, NewMVReg);
        false ->
            {ok, {merge, MVReg}}
    end;
...
%% kept.
downstream({propagate, {Value, TS, _Actor}}, MVReg ) ->
    {ok, {merge, merge(MVReg, init(Value, TS))}}.

update({merge,  MVreg1}, MVreg2) ->
    {ok, merge(MVreg1, MVreg2)}.
...
-spec from_binary(binary()) -> {ok, mvreg()} | {error, ?UNSUPPORTED_VERSION} | ?INVALID_BINARY.
from_binary(<<?TAG:8/integer, ?V1_VERS:8/integer, Bin/binary>>) ->
    {ok, riak_dt:from_binary(Bin)};
from_binary(<<?TAG:8/integer, Vers:8/integer, _Bin/binary>>) ->
    {error, ?UNSUPPORTED_VERSION(Vers)};
from_binary(_B) ->
    ?INVALID_BINARY.
...
is_operation(Operation) ->
    case Operation of
        {assign, {_, _Actor}} ->
            true;
        {assign, {_, Number, _Actor}} ->
            (is_integer(Number) andalso (Number >= 0));
        {propagate, {_, _, _Actor}} ->
            true;
        _ ->
            false
    end.
...
apply_update(Op, MVreg) ->
    {ok, Downstream} = downstream(Op, MVreg),
    update(Downstream, MVreg).
...
basic_assign_test() ->
    ...
    {ok, MVReg1} = apply_update({assign, {value0, actor0}}, MVReg0),
    ?assertEqual([{value0, VC1}], MVReg1),
    {ok, MVReg2} = apply_update({assign, {value1, actor0}}, MVReg1),
...
update_assign_withts_test() ->
    ...
    {ok, MVReg1} = apply_update({assign, {value1, 2, actor1}}, MVReg0),
    ?assertEqual([{value1, VC2}], MVReg1),
    %% Update a MVReg with a lower timestamp than the current one; should have no effect.
    {ok, MVReg2} = apply_update({assign, {value0, 1, actor1}}, MVReg1),
    ?assertEqual([{value1, VC2}], MVReg2),
    %% Test again with higher timestamp for actor1
    {ok, MVReg3} = apply_update({assign, {value2, 4, actor1}}, MVReg2),
    ?assertEqual([{value2, VC4}], MVReg3),
    %% Test with actor2
    {ok, MVReg4} = apply_update({assign, {value3, 2, actor2}}, MVReg3),
...
update_diff_actor_test() ->
    ...
    {ok, MVR1} = apply_update({assign, {value1, actor1}}, MVR0),
    {ok, MVR2} = apply_update({assign, {value2, actor2}}, MVR1),
    Value = value(MVR2),
...
propagate_test() ->
    ...
    VC1 = riak_dt_vclock:increment(actor1, VC0),
    VC2 = riak_dt_vclock:increment(actor2, VC0),
    {ok, MVReg1_1} = apply_update({assign, {value1, actor1}}, MVReg1_0),
    %% Propagate a timestamp that is not compatible with the current ones
    {ok, MVRMerge1} = apply_update({propagate, {value2, VC2, nothing}}, MVReg1_1),
    ...
    {ok, MVRMerge2} = apply_update({propagate, {value2, VC12, nothing}}, MVRMerge1),
    ...
    {ok, MVRMerge3} = apply_update({propagate, {value3, VC1, nothing}}, MVRMerge2),
...
update_assign_diverge_test() ->
    ...
    {ok, MVReg1} = apply_update({assign, {value3, actor1}}, MVReg0),
    ...
    {ok, MVReg2} = apply_update({assign, {value4, actor3}}, MVReg0),
...
roundtrip_bin_test() ->
    MVReg = new(),
    {ok, MVReg1} = apply_update({assign, {2, a1}}, MVReg),
    {ok, MVReg2} = apply_update({assign, {4, a2}}, MVReg1),
    {ok, MVReg3} = apply_update({assign, {89, a3}}, MVReg2),
    {ok, MVReg4} = apply_update({assign, {<<"this is a binary">>, a4}}, MVReg3),
...
is_operation_test() ->
    ...
    ?assertEqual(true, is_operation({assign, {something, 20, actor}})),
    ?assertEqual(false, is_operation({assign, {something, some_value, actor}})),
